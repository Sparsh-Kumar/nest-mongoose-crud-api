...
describe('Users Service [Unit Test]', function () {
        ...
        UsersService,
        {
          ...
          useFactory: () => ({
            findOne: jest.fn(async () => {
              const randomUser: LooseObject = <LooseObject>(
  ...
  test('createMagicLink - User Already Exists.', async function () {
    ...
    await expect(
      usersService.createMagicLink(userSignUpBody),
  ...
  test('signIn', async function () {
    ...
    const tokenDetails: TokenDetails = <TokenDetails>(
      await usersService.signIn(userSignInBody)
    );
